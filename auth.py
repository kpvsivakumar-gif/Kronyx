...
from security import generate_api_key, hash_password, verify_password, is_valid_key_format, create_access_token, is_login_blocked, record_login_attempt, get_remaining_block_time
...
def validate_api_key(api_key):
    ...
    if not is_valid_key_format(api_key):
        log_security("invalid_key_format")
        return None
    user = user_get_by_key(api_key)
    if not user:
        log_security("invalid_key")
        return None
    return user
