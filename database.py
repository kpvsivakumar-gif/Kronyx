...
# ============================================================
# DATABASE CONNECTION - BULLETPROOF VERSION
# ============================================================
...
def _db() -> Optional[Client]:
    ...
    try:
        ...
        if not url or not key:
            log_error("SUPABASE_URL or SUPABASE_KEY not found in environment", context="db_connect")
            return None
        _client = create_client(url, key)
        return _client
    except Exception as e:
        log_error(f"DB connect error: {e}", context="db_connect")
        return None
...
def user_count_active() -> int:
    try:
        result = _db().table("users").select("id", count="exact").eq("is_active", True).execute()
        return result.count or 0
    except Exception:
        return 0
...
def user_count_by_plan(plan: str) -> int:
    try:
        result = _db().table("users").select("id", count="exact").eq("plan", plan).execute()
        return result.count or 0
    except Exception:
        return 0
...
def usage_count_by_layer(api_key: str, layer: str) -> int:
    try:
        result = _db().table("nexus_usage").select("id", count="exact").eq("api_key", api_key).eq("layer", layer).execute()
        return result.count or 0
    except Exception:
        return 0
...
def usage_by_date_range(api_key: str, start_date: str, end_date: str) -> int:
    try:
        result = _db().table("nexus_usage").select("id", count="exact").eq("api_key", api_key).gte("created_at", start_date).lte("created_at", end_date).execute()
        return result.count or 0
    except Exception:
        return 0
...
def admin_get_global_stats() -> Dict:
    try:
        users_total = _db().table("users").select("id", count="exact").execute()
        users_active = _db().table("users").select("id", count="exact").eq("is_active", True).execute()
        ...
        today_threats = _db().table("aegis_threats").select("id", count="exact").gte("created_at", today).execute()
        return {
...
def evolve_get_performance(api_key: str) -> Dict:
    try:
        result = _db().table("evolve_data").select("score, created_at").eq("api_key", api_key).order("created_at", desc=True).limit(100).execute()
        if not result.data:
