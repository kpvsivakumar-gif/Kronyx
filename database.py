...
def admin_get_global_stats():
    try:
        ...
        return {"total_users": users.count or 0, "total_memories": memories.count or 0, "total_api_calls": usage.count or 0, "total_threats_blocked": threats.count or 0}
    except Exception:
...
def evolve_get_performance(api_key):
    try:
        result = db.table("evolve_data").select("score").eq("api_key", api_key).order("created_at", desc=True).limit(100).execute()
        if not result.data:
        ...
        trend = "stable"
        if older and recent:
            if sum(recent) / len(recent) > sum(older) / len(older) + 2:
                trend = "improving"
            elif sum(recent) / len(recent) < sum(older) / len(older) - 2:
                trend = "declining"
