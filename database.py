...
try:
    if SUPABASE_URL and SUPABASE_KEY:
        ...
        log_database_event("connection", "Supabase connected successfully", success=True)
    else:
        log_error("SUPABASE_URL or SUPABASE_KEY not set", context="database_startup")
...
def user_get_by_key(api_key: str) -> Optional[Dict]:
    try:
        if not api_key:
            return None
        r1 = db.table("users").select("*").eq("api_key_1", api_key).eq("is_active", True).execute()
        if r1.data:
            return r1.data[0]
        r2 = db.table("users").select("*").eq("api_key_2", api_key).eq("is_active", True).execute()
...
def user_exists(email: str) -> bool:
    try:
        result = db.table("users").select("id", count="exact").eq("email", email.lower().strip()).execute()
...
def notification_mark_all_read(api_key: str) -> bool:
    try:
        db.table("notifications").update({"read": True}).eq("api_key", api_key).eq("read", False).execute()
        return True
    except Exception:
        return False
...
def notification_delete(notification_id: str, api_key: str) -> bool:
    try:
        db.table("notifications").delete().eq("id", notification_id).eq("api_key", api_key).execute()
        return True
    except Exception:
        return False
...
def memory_stats_increment_total(api_key: str) -> None:
    try:
        ...
        if existing.data:
            ...
            db.table("memory_stats").update({
                "total_ever": current + 1,
                "updated_at": datetime.utcnow().isoformat()
            }).eq("api_key", api_key).execute()
        else:
            db.table("memory_stats").insert({
                "api_key": api_key,
                "total_ever": 1,
                "deleted_count": 0
            }).execute()
