        ...
        if content_length:
            try:
                ...
                if size > MAX_REQUEST_SIZE_BYTES:
                    ...
                    return JSONResponse(status_code=413, content={"error": "Request too large", "max_size_bytes": MAX_REQUEST_SIZE_BYTES})
            except ValueError:
                pass
...
class IPRateLimitMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        ...
        if not record_ip_request(client_ip):
            return JSONResponse(status_code=429, content={"error": "Too many requests from this IP", "retry_after": "60 seconds"})
        return await call_next(request)
